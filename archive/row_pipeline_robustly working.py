...
def logic_sql_pipeline(query, tables):
    ...
    # num_filter=llm_json(f"How many WHERE clause does this sql query have? {sql_query}",response_type=int)
    ...
    for i in combined_list:
        ...
        response=ask_llm(f"Update the query {updated_query} with the information from the filtering opeartion. In the filtering opeation now also include via an or-statement {candidates_with_true_boolean}")
