...
def logic_sql_pipeline(query, tables):
    ...
    #print(f"The context is {context}")
    #Get SQL query
    response = ask_gemini(f"Convert the following query to SQL: {query}. Don't rename any of the tables in the query. The structure of the database is the following: {context}.")
    ...
    num_filter=gemini_json(f"How many filters are applied to this query? Filters are expressed trhough the filtering procedure. {sql_query}",response_type=int)
    ...
    input_prompt=f'''For each applied filter. The number of applied filters should be {num_filter}, give me the an executable SQL query to identify the values for the columns on which a filtering operation is performed. Make sure to retrieve every filtering opearation seperately
                        ...
                        '''
    # Filtering list
    queries=gemini_json(prompt=input_prompt,response_type=list[QUERY])
    ...
    input_prompt=f'''For each filtering operation, give me the an executable SQL query to identify the values for the columns on which a filtering operation is performed.
                        ...
                        Output:
                        '''
    categories=gemini_json(prompt=input_prompt,response_type=list[CATEGORY])
    ...
    for i in combined_list:
        ...
        boolean_vector=gemini_json(prompt,response_type=list[bool])
        ...
        response=ask_gemini(f"Update the query {updated_query} with the information from the filtering opeartion. In the filtering opeation now also include via an or-statement the following values {candidates_with_true_boolean}")
    ...
    # response=ask_gemini(f"First let's deal with Filtering operation. Is there any in this query. If yes, give me the filtering operation in combination with the relevant table as a SQL query. If no, write 'No filtering operation'. Input: {sql_query}")
    # print(f"The response is {response}")
    # filter=extract(response, start_marker="```sql",end_marker="```" )
    # print(f"The filter applied is {filter}")
    # general_statement=ask_gemini(f"Give me the following query without a 'WHERE' clause. Input: {sql_query}")
    ...
    # general_statement=ask_gemini(f"Give me the following query without a 'WHERE' clause. Input: {sql_query}")
    # intent=ask_gemini(f"Formulate the intent of this filtering operation {filter} for this query {sql_query} in one sentence.")
    # print(f"The intent is  {intent}. ")
    # table_wothout_filter=ask_gemini(f"Give me the sql query name without the filtering condition.  {filter}")
    ...
    # sql_filter=ask_gemini(f"Give me SQL query to retrieve the rows from the table needed  for this {filter}. From this {sql_query} with that intent {intent},")
    ...
    # subqueries = ask_gemini(f'''If possible, rewrite this query such, that complicated statements are written into subqueries .Make sure both queries return the same result. Input: {sql_query}''')
    # subqueries= extract(subqueries, start_marker="```sql",end_marker="```" )
    # print(f"The subqueries are {subqueries}")

    # ordered_subqueries = ask_gemini(f'''Give me a order of the queries such that I can execute them in a specific order please! Input: {subqueries}''')
